...

{
    ...
    public bool GroundCheck()
    {
        ...
        if (hit.collider != null)
        {
            transform.rotation = Quaternion.Slerp(transform.rotation, Quaternion.LookRotation(Vector3.Cross(transform.right, hit.normal), hit.normal), Time.smoothDeltaTime * 50f);
            return true;
        }
        else
        {
